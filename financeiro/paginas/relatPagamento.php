<?php
	// Relatorio por forma de pagamento - filtros padrao + tabela RetornaSomaPagamentoPeriodo
	...
	$pagamento = (isset($_POST['txtPagamento'])) ? $_POST['txtPagamento'] : '';
...
<div class="space-y-6">
	...
	<header>
		<h1 class="text-xl font-semibold text-slate-900">Relatório por Forma de Pagamento</h1>
		<p class="mt-1 text-sm text-slate-500">Volume e ticket médio por meio de recebimento</p>
	</header>

	<?php renderRelatTabs('relatPagamento'); ?>

	<form action="?s=relatPagamento" method="post" class="bg-white rounded-xl ring-1 ring-slate-200 p-6">
		<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
			...
			<div>
				<label for="txtPagamento" class="block text-sm font-medium text-slate-700 mb-1.5">Forma de Pagamento</label>
				<select class="block w-full px-3 py-2 text-sm border border-slate-300 rounded-lg focus:ring-2 focus:ring-brand-500 focus:border-brand-500" id="txtPagamento" name="txtPagamento">
					<?php $cadastro->ListaFormaPagamentoFormRelat($pagamento); ?>
				</select>
			</div>
			<div>
	...
	<section class="bg-white rounded-xl ring-1 ring-slate-200 overflow-hidden">
		<?php $relatorio->RetornaSomaPagamentoPeriodo($dataInicio,$dataFim,$pagamento); ?>
	</section>
